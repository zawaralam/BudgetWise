...
async function getAppointments(username) {
    // get the appointments of a user
    var conn = await connect();
    let user = await conn.collection('users').find(
        {username: username}
    ).toArray();
    let appointments = user[0].appointmentTimes;
    return appointments;
}
